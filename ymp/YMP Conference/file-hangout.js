...
function setUserInterface() {
    ...
    fileElement.onchange = function() {
        ...
        var div = quickOutput('Сейчас отправляется:', html);

        FileSender.send({
            channel: hangoutUI,
            file: file,
            onFileSent: function(file) {
                quickOutput(file.name, 'успешно отправлен');
                disable(false);
                statusDiv.innerHTML = '';
...
var config = {
    ...
    onRoomFound: function(room) {
        ...
        tr.innerHTML = '<td>' + room.roomName + '</td>' +
            '<td><button class="join" id="' + room.roomToken + '">Войти</button></td>';
...
function onMessageCallback(data) {
    if (data.connected) {
        quickOutput('Пользователь подключен');
        return;
    }
    ...
    fileReceiver.receive(data, {
        onFileReceived: function(fileName) {
            quickOutput(fileName, 'прислан успешно');
            disable(false);
            statusDiv.innerHTML = '';
        },
        onFileProgress: function(e) {
